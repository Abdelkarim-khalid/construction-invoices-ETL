        ...
        with st.form("new_proj"):
            ...
            if st.form_submit_button("حفظ") and name:
                try:
                    ...
                    else: st.error(f"خطأ: {res.text}")
                except Exception as e: st.error(f"استثناء: {str(e)}")
...
# ==========================================
    ...
    if st.session_state.step == 1:
        ...
        st.session_state.wiz_invoice_no = c_inv.number_input("رقم المستخلص", min_value=1, value=st.session_state.wiz_invoice_no)
        st.session_state.start_date = c_d1.date_input("من تاريخ", value=date.today())
        st.session_state.end_date = c_d2.date_input("إلى تاريخ", value=date.today())
        ...
        with col_back: st.button("⬅️ السابق", disabled=True)
        with col_next:
            if st.button("التالي ➡️", use_container_width=True):
                if not st.session_state.get('uploaded_file'): st.error("يجب رفع ملف!")
                elif not st.session_state.sheet_civil and not st.session_state.sheet_elec: st.error("اختر شيت واحد على الأقل!")
                else:
    ...
    # --- الخطوة 2 ---
        ...
        header_row = st.number_input("📍 رقم صف العناوين في الإكسيل:", min_value=1, value=10)
        ...
        with col_next:
            if st.button("معالجة وحفظ ✅", use_container_width=True):
                ...
                # --- الدالة المحدثة مع تفاصيل الخطأ وتحديد التخصص ---
                def send_sheet(dataframe, mapping, trade_arg):
                    ...
                    # هنا نرسل نوع التخصص (civil / elec)
                    data = {
                    ...
                    try:
                        ...
                        if res.status_code == 200:
                            ...
                            # محاولة قراءة الخطأ بالتفصيل من السيرفر
                            try:
                ...
                if st.session_state.sheet_civil:
                    ...
                    if is_ok: success_log.append(f"المدني: {msg}")
                    else: errors_log.append(f"❌ خطأ مدني: {msg}")

                if st.session_state.sheet_elec:
                    is_ok, msg = send_sheet(elec_df, st.session_state.mapping_elec, "elec")
                    if is_ok: success_log.append(f"الكهرباء: {msg}")
                    else: errors_log.append(f"❌ خطأ كهرباء: {msg}")

                # عرض النتائج
                if errors_log:
                    for err in errors_log:
                        st.error(err)  # سيعرض رسالة الخطأ المفصلة هنا
...
# ==========================================
    ...
    with tab_rev:
        ...
        iid = col_in.number_input("رقم المستخلص للمراجعة", value=1)
            ...
            if st.button("عرض التقرير"):
                try:
                    ...
                    if res.status_code == 200:
                        ...
                        else: st.info("لا توجد بيانات لهذه الفترة.")
